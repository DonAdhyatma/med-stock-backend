...
export const Tampil = async (req, res) => {
    const sql = `
        SELECT TBK.*, TB.nama_barang, TC.nama_customer 
        FROM tbl_barang_keluar TBK 
        JOIN tbl_customer TC ON TC.id = TBK.id_customer 
        JOIN tbl_barang TB ON TB.id = TBK.id_barang
    `;
...
export const TampilObat = async (req, res) => {
...
export const Detail = async (req, res) => {
    const sql = `
        SELECT TBK.*, TB.nama_barang, TB.harga, TC.nama_customer, TC.nohp_customer, TC.alamat_customer 
        FROM tbl_barang_keluar TBK 
        JOIN tbl_customer TC ON TC.id = TBK.id_customer 
        JOIN tbl_barang TB ON TB.id = TBK.id_barang 
        WHERE TBK.id = ?
...
export const Tambah = async (req, res) => {
    ...
    db.query(barang, [req.body.id_barang], (err, data) => {
        ...
        const insert = "INSERT INTO tbl_barang_keluar (`id_customer`,`id_barang`,`tgl_keluar`,`stok_keluar`,`tgl_kedaluwarsa`,`total`) VALUES (?)";
        const values = [
            req.body.id_customer,
            req.body.id_barang,
            req.body.tgl_keluar,
            req.body.stok_keluar,
            req.body.tgl_kedaluwarsa,
            total
        ];
        db.query(insert, [values], (err, result) => {
            ...
            db.query(sql, [updatestok, id], (err, result) => {
                if (err) return res.json({ Error: "Error inside server" });
